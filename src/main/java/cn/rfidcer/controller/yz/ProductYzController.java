 ...
 */
@Controller
@RequestMapping("/product_yz")
public class ProductYzController {
	...
	@Autowired
	private ProductYzService productServiceYz;
	/**
	 * 新增或修改产品;
	 * @param user
	 * @param product
	...
	public ResultMsg addProduct(@CurrentUser User user, Product product) {
		return productServiceYz.addOrUpdateProduct(product, user);
	}
	 ...
	 * @return
	 */
	@RequestMapping("/getShareCompany")
	@ResponseBody
	public List<Product> getShareCompany(Page page, Product product) {
		List<Product> lst  = productService.findProductListByGoodsVariable(page, product);
		return lst;
	}
