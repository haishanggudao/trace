 ...
 */
@Controller
@RequestMapping("/product_fruit")
public class ProductFruitController {

	@Autowired
	private ProductFruitService productFruitService;
	...
	public ResultMsg addProduct(@CurrentUser User user, Product product) {
		return productFruitService.addOrUpdateProduct(product, user);
	}
	...
	public ResultMsg delProduct(Product product) {
		return productFruitService.delProduct(product);
	}
	...
	public List<Product> findProductListByGoodsVariable(Page page, Product product) {
		List<Product> lst  = productFruitService.findProductListByGoodsVariable(page, product);
		return lst;
	}
	...
	public List<Product> list(Page page, Product product) {
		return productFruitService.getProductList(page, product);
	}
	...
	public List<Product> findProductInfoAreDetailed(Product product) {
		return productFruitService.findProductInfoAreDetailed(product);
	}
	...
	public Map<String, Object> findAllList(Page page, Product product) {
		List<Product> lst = productFruitService.getProductList(page, product);
	...
	public Map<String, Object> findAllQueryList(Page page, Product product) {
		...
		map.put("rows", productFruitService.findProductListByQuery(page, product));
	...
	public ResultMsg importProductCategory(MultipartFile uploadImportFile, @CurrentUser User currentUser,
		...
		return productFruitService.addImportProduct(uploadImportFile, currentUser, companyId, productType);
	}
	...
	public ResultMsg transform(Product product, @CurrentUser User currentUser) {
		return productFruitService.changeTransform(product);
	}
}