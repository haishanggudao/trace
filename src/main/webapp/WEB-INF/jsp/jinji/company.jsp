...
<table id="itemsgrid" title="金机快餐-公司列表" style="width: 100%;"></table>
			...
			<div title="可选" style="padding: 5px;">
				...
				<div class="fitem">
					...
					<input class="easyui-combobox" name="cnature" style="width: 180px;" 
							data-options="prompt:'企业性质',valueField:'varId',textField:'varValue',url:'jinji_company/getCompanyNatures'">
					&nbsp;
				</div>
				<div class="fitem">
			...
			<div title="企业宣传" style="padding: 5px;">
			      <div class="fitem"  style="float:left;width:200px;margin-left: 10px">
							<div class="easyui-tabs" id="productImgTabs" style="width:175px;height:205px;" data-options="plain:true,narrow:true,pill:false,justified:false">
							    <div title="营业执照" style="padding:5px;"  >
							       	<img id="company_image" width="160px" height="160px"  border="1"  style="border-color:#ccc">
							    </div>
							    <div title="其他证件" style="padding:5px;" >
							       <img id="company_publicityImage" width="160px" height="160px"  border="1"  style="border-color:#ccc">
							    </div>
							</div>
						</div>
				<div class="fitem">
					          <label class="fitemlabel width_100" >营业执照：</label>
							  ...
							  <a href="#" class="easyui-linkbutton" style="height: 22px" 
							  		onclick="javascript: $('#companyImgFilebox').filebox('setValue', '');$('#company_image').attr('src','');">清空</a>
					</div>
					<div class="fitem">
					          <label class="fitemlabel width_100">其他证件：</label>
					          <input id="publicityImageFilebox"  name="companyImgchyienelicensefile" class="wu-text"  style="width: 320px;"/>
							  <input type="hidden" name="chygienelicenseUrl">
							  <a href="#" class="easyui-linkbutton" style="height: 22px" 
							  		onclick="javascript: $('#publicityImageFilebox').filebox('setValue', '');$('#company_publicityImage').attr('src','');">清空</a>
					</div>
		</div>
	</form>
</div>
...
function ajaxLoading(){
 ...
 function ajaxLoadEnd(){
	...
	function reloadCompany() {
		$('#itemsgrid').datagrid({  
		    url:'jinji_company/findAllList?findType=1',  
		    queryParams:{
	...
	function searchform() {
		$('#itemsgrid').datagrid({  
		    url:'jinji_company/findAllList?findType=2',  
		    queryParams:{
	...
	$("#publicityImageFilebox").filebox({
		...
		onChange : function() { 
			if($(this).filebox('getValue')!=""){
				$(this).filebox('uploadPreview',{ imgId: "company_publicityImage" });
			}
		},
		onClickButton: function() { 
				$('#companyImgTabs').tabs('select',1);
			...
			try {
				...
				setTimeout(function() {
					$('#itemForm').form('load', {
						...
						chygienelicenseUrl : row.chygienelicenseUrl,
						licenseUrl : row.licenseUrl
					});
				}, 500);
				/**营业执照
				*/
				$("#companyImgFilebox").filebox("setText", row.licenseUrl);
				if(row.licenseUrl!= null || row.licenseUrl != ""){
					$("#company_image").attr('src', row.licenseUrl);
				}else{
					$('#company_image').attr('src','static/img/defaultImg.jpg');
				}
				/**其他证件
				*/
				$("#publicityImageFilebox").filebox("setText", row.chygienelicenseUrl);
				if(row.chygienelicenseUrl!= null || row.chygienelicenseUrl != ""){
					$("#company_publicityImage").attr('src', row.chygienelicenseUrl);
				}else{
					$('#company_publicityImage').attr('src','static/img/defaultImg.jpg');
				}
