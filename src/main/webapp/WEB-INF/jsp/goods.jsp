...
<script type="text/javascript" src="static/js/jquery-form-download.js"></script>
		...
		<div id="wu-toolbar" style="padding: 5px 0;">
			<div class="wu-toolbar-button">
				...
				<shiro:hasPermission name="base:goods:createQRCode">
					<a href="#" class="easyui-linkbutton" iconCls="icon-codeqr" onclick="openQRCode()" plain="true">生成二维码</a>
					<span style="color: #D3D3D3">| </span>
				</shiro:hasPermission>
				...
				<shiro:hasPermission name="base:goods:printQRCodeByPlugin">
					<a href="#" class="easyui-linkbutton" iconCls="icon-codeqr" onclick="createQRCodeByPlugin()" plain="true">打印二维码(插件)</a>
					<span style="color: #D3D3D3">| </span>
				</shiro:hasPermission>
				
				<shiro:hasPermission name="base:goods:printBarCode">
					<a href="#" id="createBarcode" class="easyui-linkbutton" data-options="menu:'#createbarcode',iconCls:'icon-codeqr'"  onclick="printBarcode()" plain="true">打印条形码</a>
					<span style="color: #D3D3D3">| </span>
				</shiro:hasPermission>
				...
				&nbsp;&nbsp;&nbsp;
				<label class="fitemlabel width_60">创建日期:</label> 
					<input id="createTimeOne" value="new Date">
							<label>至</label>
					<input id="createTimeTwo" value="new Date">
...
<!-- 			</div> -->
		...
		<div id="printPluginDownload" class="easyui-dialog" title="打印插件下载"
			data-options="modal:true,closed:true"
			style="width: 330px; padding: 20px;">
			<div>
				点击下载打印插件,安装完成后需刷新页面!<a class="easyui-linkbutton" href="static/CLodopPrint_Setup_for_Win32NT.zip" target="_self">执行安装</a>
			</div>
		</div>
...
<div   data-options="modal:true,closed:true">
	<div id="qrcode_print">
	</div>
</div>
<script type="text/javascript">
	...
	$(function() {
		...
		$("#createTimeOne").datebox({
			prompt:'开始日期',
			onSelect:function(data){
				$('#createTimeTwo').datebox('calendar').calendar({
		...
		$('#createTimeTwo').datebox({
			prompt:'结束日期',
			onSelect:function(data){
				$('#createTimeOne').datebox('calendar').calendar({
					validator: function(date){
							return date<=data;
					}
				});
	...
	function CreatePrintPageOne(productName,qrCodeContent){
    	LODOP.NewPage();
		LODOP.ADD_PRINT_BARCODE(5,75,70,70,"QRCode",qrCodeContent);
		LODOP.SET_PRINT_STYLEA(0,"QRCodeVersion",3);
		LODOP.SET_PRINT_STYLEA(0,"Horient",2);
		LODOP.ADD_PRINT_TEXT(66,4,205,20,productName);
	...
	function openQRCode() {
		...
		if (checkedItems.length == 1) {
			...
			$('#downMultiImgCount').combobox('setValue', '2');
		} else {
			$.messager.alert('信息提示', '亲,请选择一个商品,暂时不可以选择多个的！', 'info');
		}
	}
	...
	function createQRCode(){
		...
		if (checkedItems.length == 1) {
			...
			$('#multiImgCount').combobox('setValue', '2');
		} else {
			$.messager.alert('信息提示', '亲,请选择一个商品,暂时不可以选择多个的！', 'info');
		}
	}
	function createQRCodeByPlugin(){
		var checkedItems = $('#goodsList').datagrid('getChecked');
		if (checkedItems.length == 1) {
			LODOP=getLodop();
			if(!LODOP){
				$('#printPluginDownload').dialog({
					closed : false,
					modal : true,
					iconCls : 'icon-codeqr'
				});
				...
				$.post("goods/getQRCode",param,function(data){
			    	LODOP.PRINT_INITA(0,0,220,100,"商品二维码");
			    	
					for(var i=0;i<data.length;i++){
				    		CreatePrintPageOne(row.product.productName, data[i].code);
					}
					LODOP.PREVIEW();
				});
			}
	...
	function printQRCode(){
		...
		$.post("goods/getQRCode",param,function(data){
			...
			$("#qrcode_print").find("canvas").each(function(){
				...
				if(multiImgCount==1){
					...
					imgObj=$("<img style='margin:20px auto 15px auto;'>").attr("src",img);
				}else{
					div=$("<div style='float:left;'></div>");
					imgObj=$("<img style='margin:10px 10px 10px 15px;'>").attr("src",img);
				}
				var div2=$("<div style='text-align:center;'></div>");
				div2.text(row.product.productName);
				var center=$("<center></center>")
				center.append(imgObj).append(div2)
				div.append(center);
	...
	function downloadCode() {
		...
		if (rows.length >= 1) {
			...
			var postParams={"strJson":encodeURI(JSON.stringify(lstGoods)),"companyId":vcompanyId};
			formDownLoadByJson("goods/downloadcode",postParams);
		} else {
			$.messager.alert('信息提示', '请选择一个商品！', 'info');
		}
	}
		...
		$('#goodsList').datagrid(
				{
					...
					onBeforeLoad : function(param) {
						...
						param.createTimeOne = $('#createTimeOne').datebox('getValue')+" 00:00:00";
						param.createTimeTwo = $('#createTimeTwo').datebox('getValue')+" 23:59:59";
